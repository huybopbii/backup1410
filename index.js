...
if (process.env.AUTOCLEAN == 'on') setTimeout(() => {
	console.log("Đang dọn dẹp bộ nhớ sau 10 phút!");
	require("node-cmd").run("rm -rf app/handle/src/video.mp4") && require("node-cmd").run("npm cache clean --force") && require("node-cmd").run("rm -rf .git");
}, 600000);
