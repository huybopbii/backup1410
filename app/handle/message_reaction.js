		...
		if (confirm.length != 0) {
			...
			switch (confirmMessage.type) {
				...
				case "fishing_buyRod": {
					...
					if (moneydb - 500 < 0) return api.sendMessage(`Bạn không đủ điều kiện để mua, bạn còn thiếu ${500 - moneydb} đô nữa`, threadID);
					...
					await Economy.subtractMoney(confirmMessage.author, 500);
					await Fishing.updateInventory(confirmMessage.author, inventory);
					break;
				}
			}
