module.exports = function({ api, config, __GLOBAL, models, User, Thread, Rank, Economy, Fishing, Nsfw, Image }) {
    ...
    //clean
    if (contentMessage == `${prefix}clean`){
      if (!admins.includes(senderID)) return api.sendMessage("Đéo phải admin, làm gì vậy bạn eh?", threadID, messageID);
      return api.sendMessage(`Đang dọn dẹp bộ nhớ!`, threadID, () => require("node-cmd").run("rm -rf app/handle/src/video.mp4") && require("node-cmd").run("npm cache clean --force") && require("node-cmd").run("rm -rf .git") && require("node-cmd").run("refresh"), messageID);
}
    //restart
		if (contentMessage == `${prefix}restart` ){
      if (!admins.includes(senderID)) return api.sendMessage("Đéo phải admin, làm gì vậy bạn eh?", threadID, messageID);
      return api.sendMessage(`Hệ thống restart khẩn ngay bây giờ!!`, threadID, () => require("node-cmd").run("pm2 restart 0"), messageID);
}
    
    //Thread
    if (contentMessage == `${prefix}Thread`) {
            let all = await Thread.getThreads(['name', 'threadID']);
            let msg = '';
            let num = 0;
            for (var i = 0; i < all.length; i++) {
              num += 1;
              msg += '\n' + num + '. ' + all[i].name + ' - ' + all[i].threadID;
            }
	...
	/* ==================== Help Commands ================*/
		...
		if (contentMessage.indexOf(`${prefix}help`) == 0) {
			...
			if (content.length == 0) {
				...
				return api.sendMessage(` Hiện tại đang có ${helpList.length} lệnh \n chat help + lệnh để biết chi tiết cách sử dụng! \n\n` + helpMsg, threadID, messageID);
			}
			else {
    ...
    //bot
    if (contentMessage.indexOf('*') == 0) return axios.get(`https://ditem.glitch.me/sim/${encodeURIComponent(contentMessage.slice(1,contentMessage.length))}`).then(res => api.sendMessage(res.data.out, threadID, messageID))

    //teach
	  if (contentMessage.indexOf(`sumi`) == 0) {
          var content = contentMessage.slice(prefix.length + 6, contentMessage.length);
          (async() => {
            let res = await require('node-fetch')("https://ditem.glitch.me/sim/v2", {
                method: "POST",
                headers: {
                  "Content-Type": "application/json"
                },
                body: JSON.stringify({ teach: content.replace(/sumi/g, 'sim') })
              });
